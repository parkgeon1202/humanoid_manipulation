...
// /vision(Vision2MasterMsg, 비전 노드의 원본 출력)을 직접 구독하되, main_node.cpp에
// 바로 물릴 수 있도록 실제 master가 하는 역할(비전 -> /master2mani 릴레이 +
// activate_cmd 발행)만 최소한으로 재현함. main_node.cpp는 그대로 두고 이 노드만
// 새로 띄우면 실제 master/task_planner 없이도 전체 파이프라인을 테스트할 수 있음.
//
//  - /vision(ball_cam_x/y/z, mm 단위)을 받으면 공의 x,y,z를 m로 변환해
//    /master2mani(Point32, main_node.cpp가 구독하는 것과 동일한 토픽/타입)로
//    그대로 릴레이함(debug_ik_node.cpp의 onVision과 동일한 이유로 mm->m 변환 필요).
...
//    false를 보내는데, 그걸 받으면 터미널에 로그로 알려줌(activate_cmd_pub_/sub_는
...
class MasterNode : public rclcpp::Node {
 ...
 private:
  void onVision(const Vision2MasterMsg::SharedPtr msg) {
    // /vision(ball_cam_x/y/z)은 mm 단위로 옴 - main_node/IK는 m 단위라 여기서 변환.
    Point32 out;
  ...
  void onActivateCmd(const Bool::SharedPtr msg) {
    // false는 main_node가 픽 시퀀스를 끝내고 스스로 deactivate했다는 통지 -
    // 이 노드가 방금 보낸 true는 그대로 무시(로그 대상 아님).
    if (!msg->data) {
