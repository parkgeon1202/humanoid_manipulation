...
TEST(RobotModel, LoadsUrdfAndMatchesPythonEePosition) {
  ...
  // 조인트 z-origin이 -0.15 -> -0.13으로 바뀌면서(2026-08-05) ee_pos.z()도 같이
  // +0.02 이동함(x/y는 그 조인트가 z만 바뀌어서 영향 없음).
  EXPECT_NEAR(ee_pos.x(), 0.0179, 1e-4);
  EXPECT_NEAR(ee_pos.y(), 0.1223, 1e-4);
  EXPECT_NEAR(ee_pos.z(), -0.1548, 1e-4);
}
...
TEST(IkStep, MatchesPythonGoldenTrace) {
  ...
  // z-origin이 -0.15 -> -0.13으로 바뀌면서(2026-08-05) 전체 재계산함 - 이번엔 별도
  // Python 크로스체크 없이, 이 파일의 ikStep(C++) 자체 출력을 그대로 새 golden으로
  // 씀(회귀 테스트 목적 - 이후 ikStep 동작이 의도치 않게 바뀌면 여기서 잡힘). 이
  // geometry에서는 tick9에서 damping이 min_damping(1e-6)까지 떨어지고 tick10부터
  // tol 안에 들어와 converged=true로 고정됨(그 뒤로는 q/damping 변화 없음 - ikStep의
  // early-exit).
  struct Tick {
    Eigen::VectorXd q;
    double damping;
  ...
  const std::vector<Tick> golden = {
      {v8({-0.060249321019, -0.027170976591, 0.074360434139, -0.014680407744, 0, 0, 0, 0}), 0.0005,
       false, false},
      {v8({-0.041879997875, -0.056159466749, 0.11137050884, -0.034213020457, 0, 0, 0, 0}), 0.00025,
       false, false},
      {v8({-0.014776210834, -0.078822885746, 0.12556821154, -0.050441569181, 0, 0, 0, 0}), 0.000125,
       false, false},
      {v8({0.0041392538849, -0.092852345002, 0.13027862994, -0.06074993109, 0, 0, 0, 0}), 6.25e-05,
       false, false},
      {v8({0.016381617192, -0.10115961756, 0.13143896192, -0.066992425895, 0, 0, 0, 0}), 3.125e-05,
       false, false},
      {v8({0.024366432024, -0.10612270846, 0.13133693369, -0.07080439335, 0, 0, 0, 0}), 1.5625e-05,
       false, false},
      {v8({0.029602808045, -0.10912901072, 0.13087164406, -0.07315987463, 0, 0, 0, 0}), 7.8125e-06,
       false, false},
      {v8({0.033036841745, -0.11097196906, 0.13038095157, -0.074628942929, 0, 0, 0, 0}), 3.90625e-06,
       false, false},
      {v8({0.035283801407, -0.11211293106, 0.12997272099, -0.075551644131, 0, 0, 0, 0}), 1.953125e-06,
       false, false},
      {v8({0.036749959411, -0.11282498165, 0.1296649681, -0.076134337711, 0, 0, 0, 0}), 1e-06, false,
       false},
      {v8({0.036749959411, -0.11282498165, 0.1296649681, -0.076134337711, 0, 0, 0, 0}), 1e-06, true,
       false},
      {v8({0.036749959411, -0.11282498165, 0.1296649681, -0.076134337711, 0, 0, 0, 0}), 1e-06, true,
       false},
  };
